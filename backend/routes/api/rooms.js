...
const {publish} = require('../../utils/redisPubSub'); // 경로 조정 필요
...
router.post('/', auth, async (req, res) => {
    try {
        ...
        const roomData = {
            ...
            password: undefined, // Remove sensitive information
        };

        await publish('room:created', JSON.stringify(roomData));
...
router.post('/:roomId/join', auth, async (req, res) => {
    try {
        ...
        const roomJoinData = {
            ...
            participants: populatedRoom.participants.map(participant => ({
                id: participant._id,
                name: participant.name,
                email: participant.email
            }))
        };
        await publish('room:join', JSON.stringify(roomJoinData));
